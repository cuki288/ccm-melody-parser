...
3. 코드는 해당 코드가 시작되는 첫 음에만 "c" 필드로 표시
4. 마디 구분은 가사의 흐름과 코드 진행으로 판단
5. 입력에 음높이 정보가 없으면 가사와 코드 진행을 기반으로 합리적인 멜로디를 추정
6. 섹션 라벨: "1절", "2절", "후렴", "브릿지" 등 한글 사용
7. 입력의 키와 박자표를 분석해서 key, timeSignature도 반환
